...
For an already deployed function in the same project, `flowso endpoint attach --to kapso --flow-id KAPSO_UUID --function-id FUNCTION_UUID` associates and registers it without re-uploading code/secrets. It requires an existing draft, a phone number and encryption. See [association and shared-function scope](kapso-operations.md#attach-an-existing-function-to-a-draft). `POST .../register` always registers the current association; it cannot select a function. Updating an existing endpoint during full deployment changes that function for all attached Flows, even if this command targets a draft. Use a separate development function when production must remain unaffected.
