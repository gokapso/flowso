export type ParsedArgs = {
  command: 'serve' | 'preview' | 'validate' | 'deploy' | 'send' | 'test' | 'inspect' | 'init' | 'skill' | 'catalog' | 'help';
  positional: string[];
...
export function parseArgs(argv: string[]): ParsedArgs {
  ...
  if (command && !command.startsWith('-')) {
    if (!['serve', 'preview', 'validate', 'deploy', 'send', 'help', 'test', 'inspect', 'init', 'skill', 'catalog'].includes(command)) {
